...
Menu rosalinaMenu = {
    "Rosalina menu",
    {
        { "Effettua screenshot", METHOD, .method = &RosalinaMenu_TakeScreenshot },
        { "Imposta luminosita", METHOD, .method = &RosalinaMenu_ChangeScreenBrightness },
        { "Cheats...", METHOD, .method = &RosalinaMenu_Cheats },
        { "", METHOD, .method = PluginLoader__MenuCallback},
        { "Lista processi", METHOD, .method = &RosalinaMenu_ProcessList },
        { "Opzioni di debug...", MENU, .menu = &debuggerMenu },
        { "Configurazione sistema...", MENU, .menu = &sysconfigMenu },
        { "Filtri schermo...", MENU, .menu = &screenFiltersMenu },
        { "New 3DS menu...", MENU, .menu = &N3DSMenu, .visibility = &menuCheckN3ds },
        { "Altre opzioni...", MENU, .menu = &miscellaneousMenu },
        { "Salva impostazioni", METHOD, .method = &RosalinaMenu_SaveSettings },
        { "Spegni", METHOD, .method = &RosalinaMenu_PowerOff },
        { "Riavvia", METHOD, .method = &RosalinaMenu_Reboot },
        { "Crediti", METHOD, .method = &RosalinaMenu_ShowCredits },
        { "Info debug", METHOD, .method = &RosalinaMenu_ShowDebugInfo, .visibility = &rosalinaMenuShouldShowDebugInfo },
        {},
    }
};
...
void RosalinaMenu_SaveSettings(void)
{
    ...
    {
        Draw_Lock();
        Draw_DrawString(10, 10, COLOR_TITLE, "Salva impostazioni");
        if(R_SUCCEEDED(res))
...
void RosalinaMenu_ShowDebugInfo(void)
{
    ...
    {
        Draw_Lock();
        Draw_DrawString(10, 10, COLOR_TITLE, "Rosalina -- Info debug");
        ...
        if (R_SUCCEEDED(FSUSER_GetSdmcSpeedInfo(&speedInfo)))
        {
            ...
            posY = Draw_DrawFormattedString(
                10, posY, COLOR_WHITE, "Velocita SDMC: HS=%d %lukHz\n",
        ...
        if (R_SUCCEEDED(FSUSER_GetNandSpeedInfo(&speedInfo)))
        {
            u32 clkDiv = 1 << (1 + (speedInfo.sdClkCtrl & 0xFF));
            posY = Draw_DrawFormattedString(
                10, posY, COLOR_WHITE, "Velocita NAND: HS=%d %lukHz\n",
                (int)speedInfo.highSpeedModeEnabled, SYSCLOCK_SDMMC / (1000 * clkDiv)
            );
        }
        {
...
void RosalinaMenu_ShowCredits(void)
{
    ...
    do
    {
        ...
        posY = Draw_DrawString(10, posY + SPACING_Y, COLOR_WHITE, "Traduzione ITA by M4nueleZ");
        ...
        Draw_DrawString(10, posY, COLOR_WHITE,
            (
                ...
                "  Contributori Luma3DS, contributori libctru,\n"
                "  altre persone"
            ));
...
void RosalinaMenu_Reboot(void)
{
    ...
    {
        Draw_Lock();
        Draw_DrawString(10, 10, COLOR_TITLE, "Riavvia");
        Draw_DrawString(10, 30, COLOR_WHITE, "Premi A per riavviare, premi B per annullare.");
        Draw_FlushFramebuffer();
        Draw_Unlock();
...
void RosalinaMenu_ChangeScreenBrightness(void)
{
    ...
    do
    {
        ...
        posY = Draw_DrawFormattedString(
            ...
            posY,
            COLOR_WHITE,
            "Luminosita attuale: %lu (min. %lu, max. %lu)\n\n",
            luminance,
            minLum,
            maxLum
        );
        posY = Draw_DrawString(10, posY, COLOR_WHITE, "Controlli: Su/Giu per +-1, Destra/sinistra per +-10.\n");
        posY = Draw_DrawString(10, posY, COLOR_WHITE, "Premi A per iniziare, B per uscire.\n\n");

        posY = Draw_DrawString(10, posY, COLOR_RED, "ATTENZIONE: \n");
        posY = Draw_DrawString(10, posY, COLOR_WHITE, "  * valore verra limitato dai predefiniti.\n");
        posY = Draw_DrawString(10, posY, COLOR_WHITE, "  * framebuffer inferiore verra ripristinato\nfino all'uscita.");
        Draw_FlushFramebuffer();
        Draw_Unlock();
...
void RosalinaMenu_PowerOff(void) // Soft shutdown.
{
    ...
    {
        Draw_Lock();
        Draw_DrawString(10, 10, COLOR_TITLE, "Spegni");
        Draw_DrawString(10, 30, COLOR_WHITE, "Premi A per spegnere, premi B per annullare.");
        Draw_FlushFramebuffer();
        Draw_Unlock();
...
void RosalinaMenu_TakeScreenshot(void)
{
    ...
    do
    {
        ...
        else
        {
            ...
            u32 posY = 30;
            posY = Draw_DrawString(10, posY, COLOR_WHITE, "Operazione effettuata.\n\n");
            posY = Draw_DrawFormattedString(10, posY, COLOR_WHITE, "Tempo di conversione:    %5lums\n", t1);
            posY = Draw_DrawFormattedString(10, posY, COLOR_WHITE, "Tempo di scrittura file: %5lums\n", t2);
        }
