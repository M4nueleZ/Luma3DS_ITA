...
static int ERRF_FormatGenericInfo(char *out, const ERRF_FatalErrInfo *info)
{
    static const char *types[] = {
        "generico", "corruzione", "scheda rimossa", "eccezione", "risultato fallimento", "generico (solo log)", "non valido"
    };

    static const char *exceptionTypes[] = {
        "interruzione caricamento", "interruzione dati", "istruzione non definita", "VFP", "non valido"
    };
    ...
    if(info->type == ERRF_ERRTYPE_EXCEPTION)
    {
        ...
        out += sprintf(out, "Tipo errore:       eccezione (%s)\n", exceptionType);
    }
    else
        out += sprintf(out, "Tipo errore:       %s\n", type);
    ...
    if(R_SUCCEEDED(res))
    {
        ...
        out += sprintf(out, "Nome processo:     %s\n", name);
        out += sprintf(out, "Title ID processo: %016llx\n", titleId);
    }
...
static int ERRF_FormatError(char *out, const ERRF_FatalErrInfo *info, bool isLogFile)
{
    ...
    if (isLogFile)
    {
        ...
        out += sprintf(out, "Segnalato il:      %s\n", dateTimeStr);
        dateTimeToString(dateTimeStr, timeAtBoot, false);
        out += sprintf(out, "Sistema riavviato il: %s\n\n", dateTimeStr);

    }
    switch (info->type)
    {
        case ERRF_ERRTYPE_NAND_DAMAGED:
            out += sprintf(out, "Il chip NAND e' stato danneggiato.\n");
            break;
        ...
        case ERRF_ERRTYPE_FAILURE:
            ...
            break;
        default:
            out += sprintf(out, "Dati errore irreversibile non validi.\n");
    }
...
static void ERRF_DisplayError(ERRF_FatalErrInfo *info)
{
    ...
    posY = Draw_DrawString(10, posY + SPACING_Y, COLOR_WHITE, "Premi un pulsante per riavviare.");
